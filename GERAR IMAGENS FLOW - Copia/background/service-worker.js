...
async function debugger2KDownload(tabId, imgSrc) {
  ...
  try {
    ...
    // ── Step 1: Get image position (poll up to 5s to let animation finish) ──
    let imgRect = null;
    for (let i = 0; i < 20; i++) {
      imgRect = await exec(function(src) {
        var img = Array.from(document.querySelectorAll('img')).find(function(i) { return i.src === src; });
        if (!img) return null;
        var r = img.getBoundingClientRect();
        if (r.width < 50 || r.height < 50) return null; // Ignora se a imagem ainda estiver abrindo
    ...
    if (!imgRect) {
      return { ok: false, reason: 'img not found or too small' };
    }
